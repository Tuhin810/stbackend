...
export const applicantAuth = async (req: Request, res: Response, next: NextFunction) => {
	try {
		...
		if (!(decoded.role === ROLES.applicant)) {
			return res.status(401).json({
		...
		if (req.body.user.role === ROLES.employer) {
			...
			const data = await applicantModel.findById(_id);
			if (data) {
				next();
			}
			else {
