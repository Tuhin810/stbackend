...
const CompanyRegistratioPage2 = ({ companyDetails,handleChangeCompanyDetails, stateList }: CompanyRegistrationProps2) => {
    return (
        <div>
            <div className="mb-4">
                ...
                <select id="country" name="country" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300" onChange={(e) => handleChangeCompanyDetails(e)}>
                    <option value="india" selected={companyDetails.country==="india"}>India</option>
                </select>
            </div>
            <div className="mb-4 flex">
                    ...
                    <select id="state" name="state" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300" onChange={(e) => handleChangeCompanyDetails(e)}>
                        {
                            stateList.map((state, value) => {
                                return (
                                    <option value={state} key={value} selected={companyDetails.state===state}>{state}</option>
                                )
                            })
                        }
                    </select>
                ...
                <div className="w-1/2 pl-2">
                    ...
                    <input type="number" id="pin" name="pin" className="w-full px-4 py-2 border rounded-md focus:outline-none focus:ring focus:border-blue-300" defaultValue={companyDetails.pin} onChange={(e) => handleChangeCompanyDetails(e)} />
                </div>
            </div>
            <div className="mb-4">
                <label htmlFor="address" className="block text-sm font-medium text-gray-700">Full Address</label>
                <input type="text" id="address" name="address" className="mt-1 p-2 border rounded w-full focus:outline-none focus:ring focus:border-blue-300" defaultValue={companyDetails.address} onChange={(e) => handleChangeCompanyDetails(e)} />
            </div>
        </div>
    )
}
